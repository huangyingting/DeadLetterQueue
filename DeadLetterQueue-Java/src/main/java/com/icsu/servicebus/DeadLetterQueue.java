 ...
 * <ul>
 * <li>
 * <b>Scenario 1:</b> Send a message and then retrieve and abandon the message
 * until the maximum delivery count is
 * exhausted and the message is automatically dead-lettered.
 * </li>
 * <li>
 * <b>Scenario 2:</b> Send a set of messages, and explicitly dead-letter
 * messages that do not match a certain criterion
 * and would therefore not be processed correctly. The messages are then picked
 * up from the dead-letter queue, are
 * automatically corrected, and resubmitted.
 * </li>
 * </ul>
 *
 * @see <a href=
 *      "https://docs.microsoft.com/azure/service-bus-messaging/service-bus-dead-letter-queues">Dead-letter
 *      Queue</a>
 */
public class DeadLetterQueue {
    ...
    public static void main(String[] args) {
        final DeadLetterQueue app = new DeadLetterQueue();
        app.run(args);
    }

    /**
     * Run method to invoke this demo on how to dead letter within an Azure Service
     * Bus Queue.
     */
    public void run(String[] args) {
        ...
        final String namespaceFQDN = System.getenv("NAMESPACE_FQDN");
        final String queueName = System.getenv("QUEUE_NAME");

        TokenCredential credential = new DefaultAzureCredentialBuilder().build();

        final ServiceBusClientBuilder builder = new ServiceBusClientBuilder().credential(namespaceFQDN, credential);

        try (ServiceBusSenderClient sender = builder.sender().queueName(queueName).buildClient()) {
            // Scenario 1: Dead letters a message by abandoning it until the
            // MaxDeliveryCount is exceeded.
            sendMessages(sender, 1);
            exceedMaxDelivery(builder, queueName);
            //receiveAndCompleteDeadLetterQueueMessages(builder, queueName).block();

            // Scenario 2: Dead letters a message by explicitly invoking
            // receiver.deadLetter().
    ...
    private void exceedMaxDelivery(ServiceBusClientBuilder builder, String queueName) {
        ...
        while (true) {
            IterableStream<ServiceBusReceivedMessage> msgs = receiver.receiveMessages(1, Duration.ofSeconds(2));
            if(!msgs.iterator().hasNext()){
                break;
            }
            for (ServiceBusReceivedMessage msg : msgs) {
                if (msg != null) {
        ...
        ServiceBusReceiverClient deadLetterQueueReceiver = builder.receiver()
        .queueName(queueName)
        .subQueue(SubQueue.DEAD_LETTER_QUEUE)
        .buildClient();

        IterableStream<ServiceBusReceivedMessage> deadLetterMsgs = deadLetterQueueReceiver.receiveMessages(1, Duration.ofSeconds(2));

        for (ServiceBusReceivedMessage deadLetterMsg : deadLetterMsgs) {
            if (deadLetterMsg != null) {
                System.out.printf("Received message from dead-letter queue. Sequence #: %s. DeliveryCount %d%n",
                deadLetterMsg.getSequenceNumber(), deadLetterMsg.getDeliveryCount());

                // When messages are dead lettered, there are system properties that can be set
                // to denote the
                ...
                deadLetterMsg.getDeadLetterReason(), deadLetterMsg.getDeadLetterErrorDescription(),
                deadLetterMsg.getDeadLetterSource());

                System.out.println("Application properties:");
                deadLetterMsg.getApplicationProperties()
                        .forEach((key, value) -> System.out.printf("\t%s=%s%n", key, value));
                deadLetterQueueReceiver.complete(deadLetterMsg);
            }
        } 
     ...
     *
     * <p>
     * Receive {@link ServiceBusMessage messages} and return the
     * {@link ServiceBusMessage messages} back to the queue.
     * When the max number of deliveries for each {@link ServiceBusMessage message}
     * expires, then it is moved into the
     * dead letter queue.
     * </p>
     *
     ...
     *
     * @return A Mono that completes when all messages in queue have been
     *         processed.(because a message has not been
    ...
    private Mono<Void> deadLetterByExceedingMaxDelivery(ServiceBusClientBuilder builder, String queueName) {
        // This Mono creates an async receiver, and continues receiving from that queue
        // until it has not seen a message
        // for 30 seconds.
        //
        // When it receives any messages, it abandons them so they are returned to the
        // queue to be re-received again.
        // When a message is abandoned, its delivery count is incremented. If the
        // delivery count exceeds the
        // MaxDeliveryCount for a queue, the message is placed in the deadletter queue
        // for that particular queue.
        return Mono.using(() -> {
            ...
            return receiver.receiveMessages()
                    ...
                    .flatMap(message -> {
                        System.out.printf("Received message. Sequence # %s; DeliveryCount %d%n",
     ...
     *
     * <p>
     * This method continues to receive messages from the dead letter queue, then
     * completes them.
     * </p>
     *
     * @param builder   Service Bus client builder.
     * @param queueName Name of the queue to receive from.
     *
     * @return A Mono that completes when all messages in queue have been processed
     *         (because a message has not been
     *         received in the last 30 seconds).
     */
        ...
        return Mono.using(() -> {
            ...
            return deadLetterQueueReceiver.receiveMessages()
                    ...
                    .flatMap(message -> {
                        ...
                        // In a real example, you may do some sort of computation to understand why
     ...
     *
     * <p>
     * Receives {@link ServiceBusMessage messages} and dead letters them if it has a
     * subject of "Scientist" and content
     * type of "application/json". This is to simulate that the message may be
     * malformed or didn't contain the right
     ...
     *
     * @return A Mono that completes when all the messages in the queue have been
     *         processed (because a message has not
    ...
    private Mono<Void> receiveAndDeadletterMessages(ServiceBusClientBuilder builder, String queueName) {
        return Mono.using(
                ...
                receiver -> {
                    // The scope of the receiver resource is to receive messages for a duration of
                    // `receiveDuration` then
                    ...
                    return receiver.receiveMessages()
                            ...
                            .flatMap(message -> {
                                ...
                                // Simulates that something could not be processed in the message, so we dead
                                // letter it.
     ...
     *
     * <p>
     * Receives {@link ServiceBusMessage messages} from the dead letter queue, it
     * fixes up the message then resends the
     * fixed message to the queue again. This simulates messages that may have
     * errors in them, were dead-lettered,
     * and reprocessed in the dead-letter queue so the data is correct again.
     * </p>
     *
     * @param builder        Service Bus client builder.
     * @param queueName      Name of queue to receive messages from.
     * @param resubmitSender Service Bus sender client. When messages are fixed,
     *                       they are published via this sender.
     *
     * @return A Mono that completes when all the messages in the queue have been
     *         processed (because a message has not
     *         been received in the last 30 seconds).
            ...
            return deadLetterQueueReceiver.receiveMessages()
                    ...
                    .flatMap(message -> {
                        if ("Physicist".equals(message.getSubject())) {
                            ...
                            System.out.printf(
                                    "Message resubmission is not required. MessageId = %s. SequenceNumber = %s. "
                                            + "Subject = %s%n",
                                    message.getMessageId(), message.getSequenceNumber(), message.getSubject());
                        }
